...
if libsettings then
	vmg.settings = libsettings.get_object("vmg")
	local oldvmgpath = worldpath .. "/vmg.conf"
	local oldvmg = io.open(oldvmgpath, "r")
	if oldvmg then
		oldvmg:close()
		for key, value in pairs(Settings(oldvmgpath):to_table()) do
			vmg.settings:define(key, value, false)
		end
		os.remove(oldvmgpath)
	end

	function vmg.define(flag, default)
		return vmg.settings:define(flag, default)
	end
else
